...
const wealthTranslations = {
  ...
  subtitle: { ar: 'أدلة عملية للاستثمار الحلال والميزانية وبناء الثروة', en: 'Practical guides for halal investing, budgeting, and building wealth' },
  noArticles: { ar: 'لا توجد مقالات بعد', en: 'No articles yet' },
  noArticlesDesc: { ar: 'ستظهر المقالات هنا قريباً. تابعنا للحصول على محتوى حول الاستثمار الحلال والتخطيط المالي.', en: 'Articles will appear here soon. Check back later for content about halal investing and financial planning.' },
  readArticle: { ar: '← اقرأ المقال', en: 'Read article →' },
};

const WealthIndexPage = () => {
  const { language, isRTL } = useLanguage();
  const posts = getWealthPostsByLang(language);

  const wt = (key: keyof typeof wealthTranslations) => wealthTranslations[key][language];
        ...
        <div className="grid gap-6 md:grid-cols-2">
          {posts.length > 0 ? (
              ...
              >
                ...
                <div className="p-5">
                  ...
                  <h2 className="text-lg font-semibold text-foreground group-hover:text-primary transition-colors line-clamp-2 mb-2">
                    <Link to={getWealthRoute(post.slug)}>
                      {post.title}
                    </Link>
                  </h2>
            ...
            <section className="col-span-full rounded-2xl border border-dashed border-border bg-card p-8">
              <h2 className="text-2xl font-bold text-foreground">{wt('noArticles')}</h2>
